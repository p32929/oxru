...
impl App {
    ...
    pub const SETTINGS_SECTIONS: usize = 5;

    /// Move between the dialog's sections (font / terminal FPS / dialog size
    /// / word wrap / colour). `delta` of -1 (Up / Shift+Tab) goes to the
    /// previous section, +1 (Down / Tab) to the next — both wrap around.
    ...
    /// terminal FPS, resize dialogs, toggle word wrap, or pick a colour.
    /// Every change is saved immediately (see
...
mod tests {
    ...
    fn settings_adjusts_dialog_size_within_range() {
        ...
        assert_eq!(app.dialog_size_pct, *crate::config::DIALOG_SIZE_RANGE.start());
        app.settings_adjust(60); // clamps at the ceiling
        assert_eq!(app.dialog_size_pct, *crate::config::DIALOG_SIZE_RANGE.end());
    }
