...
/// Which one-line text box a key is meant for. Each variant is one text box in
/// the UI; [`App::field`] is the only place that knows where its storage lives,
/// and [`App::field_key`] the only place that edits it.
...
/// The text width (in chars) available for word-wrap layout: the pane's own
/// width (once known — it isn't yet on the very first frame before anything
/// has rendered) minus the gutter, or effectively infinite when word wrap is
/// off or the pane width isn't known yet — either way, every line then lays
/// out as exactly one row, [`crate::buffer::Buffer::wrap_rows`]'s "no
/// wrapping" case. The single width computation shared by rendering, mouse
/// clicks, and scroll-to-cursor so they can never disagree about where a
/// line breaks.
pub(crate) fn wrap_content_width(word_wrap: bool, pane_width: Option<u16>, gutter_w: u16) -> usize {
    if !word_wrap {
        return usize::MAX;
    }
    pane_width
        .map(|w| (w.saturating_sub(gutter_w) as usize).max(1))
        .unwrap_or(usize::MAX)
}
...
pub struct App {
    ...
    /// `settings::fps_options()`. Only throttles *unattended* terminal-output-
    ...
    /// Soft-wrap long lines to the pane width instead of letting them run off
    /// the right edge (VSCode's "word wrap"). Off by default. Applies to
    /// both backends — it's about editor content, not the rendering
    /// backend.
    pub word_wrap: bool,
    /// Columns of blank margin at the left/right window edges. Non-zero only
    /// in windowed mode — see the inset in `ui::render`.
    pub gui_padding: u16,
    /// The configured padding, kept separate from the live one so the windowed
    /// entry point can switch it on without re-reading config.
    pub config_gui_padding: u16,
    /// Tint the background of the row the caret is on. On by default.
    pub current_line_hl: bool,
    /// Draw a vertical rule at each indent level. On by default.
    ...
    /// Bumped every time a project search is (re)started; carried through to
    /// the background job and echoed back on `search_rx` so a result from a
    /// query that's since been superseded (the user kept typing) is
    /// recognised as stale and discarded rather than clobbering fresher
    /// results. Same idea as `hl_pending`/`hl_cache`'s revision guard.
    search_generation: u64,
    /// Send half of the background-project-search-job channel; results land
    /// on `search_rx`. See [`Self::run_project_search`].
    search_tx: Sender<(u64, Vec<crate::search::SearchFileResult>, bool)>,
    /// Completed background search jobs, drained by
    /// [`Self::poll_search_results`] every tick.
    search_rx: Receiver<(u64, Vec<crate::search::SearchFileResult>, bool)>,
    ...
    /// Last rendered editor viewport content width (post-gutter), for
    /// word-wrap scroll math.
    ...
    /// Which history rows are ticked for a multi-entry paste, parallel to
    /// [`Self::clip_ring`]. Sized when the dialog opens and cleared when it
    /// closes — a tick is a choice you're making right now, and the ring can't
    /// change underneath it while the dialog is up.
    pub clip_checked: Vec<bool>,
    ...
    /// in production; overridden in tests (from any module — hence
    /// `pub(crate)`, not private) so they never touch the user's actual
    /// config file.
...
impl App {
    pub fn new(root: Option<PathBuf>) -> Result<Self> {
        // Under `cargo test` the global config is deliberately skipped. Tests
        // construct an `App` over a temp folder and assert on defaults; reading
        // the developer's own `~/…/oxru/config.toml` makes those assertions
        // depend on the machine they run on, so a personal `word_wrap = true`
        // fails the suite for a reason that has nothing to do with the code.
        // Project config is still read — that comes from the temp folder the
        // test itself set up.
        let config = match (&root, cfg!(test)) {
            (Some(r), false) => Config::load(r),
            (Some(r), true) => Config::load_with(None, r),
            (None, false) => Config::load_global(),
        ...
        Ok(App {
            ...
            gui_padding: 0,
            config_gui_padding: config.gui_padding(),
            current_line_hl: config.current_line(),
    ...
    pub fn open_dialog(&mut self, d: Dialog) {
        ...
        match d {
            ...
            Dialog::Clipboard => self.clip_reset_selection(),
            Dialog::Help => self.help_scroll = 0,
            Dialog::SearchFiles => {
                // Bump the generation so a job left over from a *previous*
                // session of this dialog (closed before it finished) can
                // never land in the freshly-opened, empty state.
    ...
    fn teardown_dialog(&mut self, d: Dialog) {
        match d {
            ...
            Dialog::SearchFiles => {
                ...
                // dialog is closed can't pop a "No results for …" toast (or
                // write into `results`) after the user has already moved on.
                self.search_generation += 1;
    ...
    fn persist_settings_quiet(&mut self) -> std::io::Result<()> {
        ...
        let result = crate::config::save_prefs_to(
            ...
            crate::settings::Prefs {
                ...
                fps: self.gui_fps,
                dialog_size: self.dialog_size_pct,
                word_wrap: self.word_wrap,
                accent: self.theme.accent_rgb(),
            },
        );
        if let Err(e) = &result {
    ...
    /// The number of focusable sections in the Settings dialog — the length
    /// of the table that declares them, so it can't fall out of step.
    pub const SETTINGS_SECTIONS: usize = crate::settings::SETTINGS.len();

    /// Move between the dialog's sections. `delta` of -1 (Up / Shift+Tab) goes
    /// to the previous section, +1 (Down / Tab) to the next — both wrap.
    ...
    /// The focused preference's current value, for the dialog to draw. Reading
    /// it through the table's [`Id`](crate::settings::Id) keeps `ui.rs` from
    /// naming `App`'s fields one setting at a time.
    pub fn setting_value(&self, id: crate::settings::Id) -> crate::settings::Value {
        use crate::settings::{Id, Value};
        match id {
            Id::FontSize => Value::Num(self.gui_font_size),
            Id::TerminalFps => Value::Num(self.gui_fps),
            Id::DialogSize => Value::Num(self.dialog_size_pct),
            Id::WordWrap => Value::Flag(self.word_wrap),
            Id::Accent => Value::Pick(self.settings_color),
        }
    }

    /// Left/right within the focused section. Bounds and choices come from the
    /// settings table, so the dialog can never offer a value the config file
    /// would then clamp away. Every change is saved immediately (see
    /// [`Self::persist_settings_quiet`]) so nothing is lost if the app quits
    /// before the dialog is closed normally.
    pub fn settings_adjust(&mut self, delta: i32) {
        use crate::settings::{self as st, Id};
        let Some(row) = st::SETTINGS.get(self.settings_focus) else {
            return;
        };
        match row.id {
            Id::FontSize => {
                let n = (self.gui_font_size as i32 + delta).max(0) as u32;
                self.gui_font_size = st::clamp_number(Id::FontSize, n);
            }
            Id::TerminalFps => {
                let opts = st::fps_options();
            ...
            Id::DialogSize => {
                let n = (self.dialog_size_pct as i32 + delta).max(0) as u32;
                self.dialog_size_pct = st::clamp_number(Id::DialogSize, n);
            }
            Id::WordWrap => {
                // Either direction just flips it — there's only on/off.
                if delta != 0 {
                    self.toggle_word_wrap();
                }
                return; // toggle_word_wrap already persists
            }
            Id::Accent => {
    ...
    /// Toggle soft word-wrap (VSCode-style: long lines wrap to the pane width
    /// instead of running off-screen). Persists immediately, same as every
    ...
    /// Borrow the field `id` names. The one place that knows where each text
    /// box's `(text, cursor, anchor)` actually lives.
    fn field(&mut self, id: FieldId) -> crate::editline::FieldRef<'_> {
        use crate::editline::FieldRef;
        match id {
            FieldId::FindQuery => FieldRef {
                text: &mut self.find.query,
                cursor: &mut self.find.cursor,
                anchor: &mut self.find.anchor,
            },
            FieldId::FindReplace => FieldRef {
                text: &mut self.find.replace,
                cursor: &mut self.find.replace_cursor,
                anchor: &mut self.find.replace_anchor,
            },
            FieldId::Prompt => FieldRef {
                text: &mut self.prompt.input,
                cursor: &mut self.prompt.cursor,
                anchor: &mut self.prompt.anchor,
            },
            FieldId::FileQuery => FieldRef {
                text: &mut self.file_dialog.query,
                cursor: &mut self.file_dialog.cursor,
                anchor: &mut self.file_dialog.anchor,
            },
            FieldId::SearchQuery => FieldRef {
                text: &mut self.project_search.query,
                cursor: &mut self.project_search.cursor,
                anchor: &mut self.project_search.anchor,
            },
            FieldId::TodoInput => FieldRef {
                text: &mut self.todo_input,
                cursor: &mut self.todo_input_cursor,
                anchor: &mut self.todo_input_anchor,
            },
        }
    }

    /// What has to happen after `id`'s text changes — re-run the search, refilter
    /// the file list, and so on. Fields with nothing to do say so explicitly.
    fn field_changed(&mut self, id: FieldId) {
        match id {
            FieldId::FindQuery => self.find_changed(),
            FieldId::FileQuery => self.dialog_query_changed(),
            FieldId::SearchQuery => self.search_files_query_changed(),
            // The replace box, the name prompt and the to-do input are read when
            // they're used, so there's nothing to recompute as they're typed.
            FieldId::FindReplace | FieldId::Prompt | FieldId::TodoInput => {}
        }
    }

    /// The selection in field `id`, if any.
    fn field_selection(&mut self, id: FieldId) -> Option<String> {
        let f = self.field(id);
        crate::editline::selected_text(f.text, *f.cursor, *f.anchor)
    }

    /// Run one key through the field `id` names, servicing the clipboard and the
    /// field's own change hook. Returns whether the key was consumed.
    ///
    /// Every text box in the app goes through here, so copy, cut, paste, word
    /// motion and the rest behave identically in all of them — and a fix lands
    ...
    pub fn field_key(
        ...
        use crate::editline::{self as el, Edit};
        match el::handle_key(self.field(id), key, ctrl, alt, shift) {
            Edit::Ignored => return false,
            Edit::Moved => {}
            Edit::Changed => self.field_changed(id),
            Edit::Copy => {
                if let Some(t) = self.field_selection(id) {
            ...
            Edit::Cut => {
                ...
                let f = self.field(id);
                if el::delete_selection(f.text, f.cursor, f.anchor) {
                    self.field_changed(id);
                }
            }
            Edit::Paste => {
                // Flattened to one line: these are all single-line boxes, and a
                // pasted newline would otherwise be an invisible character that
                // silently breaks the search or the filename.
                let t = one_line(self.get_clipboard());
                if !t.is_empty() {
                    let f = self.field(id);
                    el::insert_str(f.text, f.cursor, f.anchor, &t);
                    self.field_changed(id);
                }
            }
        }
    ...
    pub fn clip_paste_selected(&mut self) {
        let picked = self.clip_selection();
        if picked.is_empty() {
            return;
        }
        // Several entries paste as several lines. Joined in the order they're
        // listed — top to bottom, newest first — so what you get back is the
        // order you were looking at while ticking, not the order you happened
        // to copy them in.
        ...
        if count > 1 {
            self.notify(format!("Pasted {count} entries ({n} chars)"), ToastKind::Success);
        } else {
            self.notify(format!("Pasted {n} chars"), ToastKind::Success);
        }
    }

    /// The entries a paste would use: every ticked row in list order, or the
    /// row under the cursor when nothing is ticked.
    ///
    /// The fallback is what keeps the common case unchanged — open the
    /// history, press Enter, get the thing you were pointing at. Ticking is
    /// only for when you want more than one.
    fn clip_selection(&self) -> Vec<String> {
        let ticked: Vec<String> = self
            .clip_ring
            .iter()
            .enumerate()
            .filter(|(i, _)| self.clip_checked.get(*i).copied().unwrap_or(false))
        ...
        self.clip_ring
            .get(self.clip_cursor)
            .map(|e| vec![e.text.clone()])
            .unwrap_or_default()
    }

    /// How many rows are ticked — the footer counts them so the Enter hint can
    /// say what it's about to do.
    pub fn clip_checked_count(&self) -> usize {
        self.clip_checked.iter().filter(|c| **c).count()
    }

    /// Tick or untick the highlighted entry.
    pub fn clip_toggle(&mut self) {
        if let Some(c) = self.clip_checked.get_mut(self.clip_cursor) {
            *c = !*c;
        }
    }

    /// Clear every tick and put the cursor back at the top.
    fn clip_reset_selection(&mut self) {
        self.clip_cursor = 0;
        self.clip_checked = vec![false; self.clip_ring.len()];
    }
    ...
    /// The files on the system clipboard, if it holds files rather than text —
    /// what you get after copying something in Finder.
    ///
    /// Checked *before* the text flavour everywhere it matters: macOS puts both
    /// on the pasteboard, and the text one is often the bare filename, so
    /// reading text first would paste `a.txt` where the full path was meant.
    fn clipboard_files(&mut self) -> Option<Vec<PathBuf>> {
    ...
    pub fn paste(&mut self) {
        // A file on the clipboard pastes as its path — a document can hold the
        // text of a path, never the file itself. One per line, because that's
        // what a multi-line paste means in a text buffer (the shell wants them
        // space separated instead; see `paste_terminal`), and unescaped,
        // because backslashes here would just be noise to delete.
        let text = match self.clipboard_files() {
            Some(files) => files
                .iter()
                .map(|p| p.to_string_lossy().into_owned())
                .collect::<Vec<_>>()
                .join("\n"),
            None => self.get_clipboard(),
        };
        if text.is_empty() {
    ...
    pub fn open_path(&mut self, path: &Path) {
        ...
        // Anything that isn't editable text says so and stops here — leave any
        // open dialog up so another file can be picked. One check, in
        // `buffer::unsupported_reason`, so the message is the same wherever a
        // file is opened from.
        if let Some(reason) = crate::buffer::unsupported_reason(path) {
            self.notify(reason, ToastKind::Info);
            return;
        }
    ...
    /// `width` is the pane's content width (post-gutter, post-scrollbar) —
    /// only used when word wrap is on, to know where lines break.
    pub fn ensure_cursor_visible(&mut self, height: u16, width: u16) {
        ...
        let (row, col, cursor, line_count, scroll) = {
            let b = &self.editors[idx];
            (b.cursor_row(), b.cursor_col(), b.cursor, b.line_count(), b.scroll_row)
        };
        ...
        if cursor_moved {
            if row < next {
                // Cursor's line is above the current top — scroll up so that
                // line's own first row starts the viewport.
                next = row;
            } else {
                let b = &self.editors[idx];
                let gutter_w = (line_count.max(1).to_string().len() as u16).max(3) + 1;
                let content_width = wrap_content_width(word_wrap, Some(width), gutter_w);
                let still_visible = b
                    .wrap_position(next, row, col, content_width)
                    .is_some_and(|(row_offset, _)| row_offset < h);
                if !still_visible {
                    // Scroll down just enough that the cursor's own wrapped
                    // row becomes the last visible one: walk backward from
                    // its line, accumulating each line's wrapped-row count,
                    // stopping as soon as one more line would overflow the
                    // viewport. (With wrap off every line is exactly one
                    // row, so this reduces to the plain `row + 1 - h` a
                    // single unwrapped line would use.)
                    let mut total = 0usize;
                    let mut candidate = row;
                    loop {
                        let rows_here = b.wrap_line_ranges(candidate, content_width).len();
                        if total + rows_here > h {
                            next = (candidate + 1).min(row);
                            break;
                        }
                        total += rows_here;
                        if candidate == 0 {
    ...
    fn prompt_next_quit_blocker(&mut self) {
        let remaining = self.quit_queue.len().saturating_sub(self.quit_pos);
        // `if`, not `while`: each arm opens a prompt and returns, and the
        // answer to that prompt is what advances `quit_pos` and calls this
        // again. Written as a loop it read as though it stepped through the
        // queue itself, which it never did.
        if let Some(blocker) = self.quit_queue.get(self.quit_pos).copied() {
            match blocker {
    ...
    /// Type one character into the replace box. Goes through the shared
    /// field path, so it can't drift from what a real keystroke does.
    pub fn replace_input(&mut self, c: char) {
        let f = self.field(FieldId::FindReplace);
        crate::editline::insert(f.text, f.cursor, f.anchor, c);
        self.field_changed(FieldId::FindReplace);
    }
    ...
    #[cfg_attr(not(test), allow(dead_code))]
    /// Type one character into the find box. Goes through the shared
    /// field path, so it can't drift from what a real keystroke does.
    pub fn find_input(&mut self, c: char) {
        let f = self.field(FieldId::FindQuery);
        crate::editline::insert(f.text, f.cursor, f.anchor, c);
        self.field_changed(FieldId::FindQuery);
    }
    ...
    pub fn open_search_files(&mut self) {
        if self.root.is_none() {
            self.notify(
                format!("Open a folder first ({})", crate::keys::chord(crate::keys::id::RECENT_OPEN)),
                ToastKind::Info,
            );
            return;
        }
    ...
    #[cfg_attr(not(test), allow(dead_code))]
    /// Type one character into the search files box. Goes through the shared
    /// field path, so it can't drift from what a real keystroke does.
    pub fn search_files_input(&mut self, c: char) {
        let f = self.field(FieldId::SearchQuery);
        crate::editline::insert(f.text, f.cursor, f.anchor, c);
        self.field_changed(FieldId::SearchQuery);
    }
    ...
    /// Kick off the actual project-wide search — auto-triggered once the
    /// debounce elapses (see [`Self::poll_pending_search`]). Even with
    /// ripgrep's own (parallel, memory-mapped) search engine doing the work,
    /// scanning every matching project file off disk is still real I/O —
    /// running it synchronously here would stall input/rendering for however
    /// long that takes, which is exactly the "laggy" feeling this is meant to
    /// avoid. So, like background syntax highlighting (`highlighted_for`),
    /// this only *starts* the job on a background thread; results land on
    /// `search_rx` and are adopted by [`Self::poll_search_results`].
    pub fn run_project_search(&mut self) {
        ...
        self.project_search.in_flight = true;
        let tx = self.search_tx.clone();
        std::thread::spawn(move || {
            let (results, truncated) = search::search_project(&root, &query, false);
            let _ = tx.send((generation, results, truncated));
        });
    }
    ...
    /// whether a search job was just *started* (not finished — see
    /// [`Self::poll_search_results`]), so the GUI can force an immediate
    /// repaint to show the "Searching…" state instead of waiting for the idle
    /// floor.
    ...
    /// Adopt any project-search job that finished since the last poll. Called
    /// every tick from the main loop, alongside `poll_pending_search`.
    /// Returns whether a *result* was just adopted, so the GUI can force an
    /// immediate repaint rather than waiting for the idle floor.
    pub fn poll_search_results(&mut self) -> bool {
        let mut adopted = false;
        while let Ok((generation, results, truncated)) = self.search_rx.try_recv() {
            // A result from a query that's since been superseded (the user
            // kept typing, or closed the dialog) — discard it rather than
            // clobbering whatever's current.
            ...
            self.project_search.in_flight = false;
            adopted = true;
            if total == 0 {
    ...
    fn place_cursor_at(&mut self, col: u16, row: u16, keep_anchor: bool, alt: bool) {
        ...
        let pane_width = self.editor_panes.iter().find(|(i, _)| *i == idx).map(|(_, (_, _, w, _))| *w);
        let word_wrap = self.word_wrap;
        let buf = &mut self.editors[idx];
        let gutter_w = (buf.line_count().max(1).to_string().len() as u16).max(3) + 1;
        let content_width = wrap_content_width(word_wrap, pane_width, gutter_w);
        // Ask for rows up through the clicked one; if the buffer's shorter
        // than that (clicked below the last line), `.last()` naturally lands
        // on the last real row instead — the same "clamp to end" the old
        // line-index math did.
        let row_map = buf.wrap_rows(buf.scroll_row, content_width, view_row + 1);
        let Some(target_row) = row_map.last() else {
            return;
        };
        let click_col = col.saturating_sub(gutter_w) as usize;
        let c = target_row.start + click_col.min(target_row.end - target_row.start);
        let pos = buf.rope.line_to_char(target_row.line) + c;
        if alt {
    ...
    fn scrollbar_drag_to(&mut self, idx: usize, row: u16, grab_offset: u16) {
        let Some((_, (_, py, pw, ph))) = self.editor_panes.iter().find(|(i, _)| *i == idx).copied()
        else {
            return;
        };
        ...
        let gutter_w = (buf.line_count().max(1).to_string().len() as u16).max(3) + 1;
        let content_width = wrap_content_width(word_wrap, Some(pw), gutter_w);
        let (total, _) = buf.wrap_scroll_metrics(0, content_width);
        ...
        let target_row = ((ratio * max_scroll as f64).round() as usize).min(max_scroll);
        buf.scroll_row = buf.line_for_visual_row(target_row, content_width);
    }
    ...
    pub fn paste_terminal(&mut self) {
        // Files on the clipboard arrive as shell arguments — quoted, space
        // separated — which is what every other terminal does with them. The
        // path is what's useful at a prompt; the file's *contents* never are.
        let text = match self.clipboard_files() {
            Some(files) => Self::paths_as_shell_args(&files),
            None => self.get_clipboard(),
        };
        if text.is_empty() {
            return;
    ...
    /// Paths joined the way a shell wants them: each quoted, separated by
    /// spaces, so a multi-file copy pastes as a valid argument list.
    fn paths_as_shell_args(files: &[PathBuf]) -> String {
        files
            .iter()
            .map(|p| crate::terminalpane::shell_escape(p))
            .collect::<Vec<_>>()
            .join(" ")
    }

    /// A file dragged in from Finder / Explorer, dropped at cell `(col, row)`.
    ///
    /// Where you drop it decides what it means. Onto a terminal it's a path you
    /// were about to type, so it arrives as shell-quoted text — the same thing
    /// macOS Terminal and iTerm do. Anywhere else it's a file you want to look
    /// at, so it opens as a tab.
    ///
    /// Note this differs from *pasting* a file, which inserts the path in both
    /// places (see [`Self::paste`]): ⌘V means "put text at my cursor", while a
    /// drop means "bring this into the app".
    pub fn drop_path(&mut self, path: &Path, col: u16, row: u16) {
        if matches!(self.top_dialog(), Some(Dialog::Terminal)) {
            // In grid mode several shells are on screen at once, so the drop
            // goes to the one under the pointer rather than whichever happened
            // to be focused — the hand already said which.
            if self.terminal_grid {
                self.terminal_click(col, row);
            }
            let text = format!("{} ", crate::terminalpane::shell_escape(path));
            if let Some(term) = self.active_terminal_mut() {
                term.paste(&text);
            }
            return;
        }
        // Everywhere else — the editor, the welcome screen, or a dialog sitting
        // on top of things — a drop opens the file. `open_path` reports
        // anything that isn't editable text and leaves the UI as it was.
        self.open_path(path);
    }
    ...
    pub fn dialog_toggle_junk(&mut self) {
        ...
        self.rebuild_dialog_entries();
        self.dialog_refilter();
        let junk_key = crate::keys::chord(crate::keys::id::FD_JUNK);
        let msg = if self.dialog_show_junk {
            format!("Search now includes node_modules / build dirs ({junk_key})")
        } else {
            format!("Search hides node_modules / build dirs ({junk_key})")
        };
...
mod tests {
    ...
    /// `App::run_project_search`/`App::poll_search_results`) so typing never
    /// blocks on disk I/O — tests that need the *result*, not just the job
    /// having started, poll for it with a generous bound instead of assuming
    /// it's done synchronously.
    fn wait_for_search(app: &mut App) {
        let deadline = std::time::Instant::now() + crate::terminalpane::SHELL_WAIT;
        while std::time::Instant::now() < deadline {
            if app.poll_search_results() {
                return;
            }
            std::thread::sleep(Duration::from_millis(5));
        }
        panic!("project search did not complete within 5s");
    }
    ...
    fn opening_a_new_file_always_starts_scrolled_to_the_top() {
        ...
        app.ensure_cursor_visible(20, 80); // establishes last_cursor for this tab
        ...
        app.ensure_cursor_visible(20, 80);
    ...
    #[test]
    fn word_wrap_mouse_click_on_a_continuation_row_lands_in_that_rows_own_text() {
        let dir = workspace();
        fs::write(dir.path().join("long.txt"), "AAAA BBBB CCCC\n").unwrap();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.open_path(&dir.path().join("long.txt"));
        app.word_wrap = true;
        let idx = app.active_editor.unwrap();

        // Wrapped to 9 content chars: row 0 = "AAAA BBBB " (chars 0-10), row
        // 1 = "CCCC" (chars 10-14) — verified directly against the same
        // wrap function the click math uses, so the test doesn't duplicate
        // (and risk disagreeing with) that logic.
        let gutter_w = 4u16; // 1-line file: max(3) + 1
        let content_w = 9u16;
        let ranges = app.editors[idx].wrap_line_ranges(0, content_w as usize);
        assert_eq!(ranges, vec![(0, 10), (10, 14)], "sanity check on the wrap this test relies on");

        app.editor_panes = vec![(idx, (0, 0, gutter_w + content_w, 10))];
        let body_offset = app.tab_strip_rows() + if app.breadcrumb_row_shown() { 1 } else { 0 };

        // Click column gutter_w+1 on the SECOND visual row (view_row 1) —
        // must land inside "CCCC" (chars 10-14), not be mistaken for a
        // position on row 0's "AAAA BBBB ".
        app.place_cursor_at(gutter_w + 1, body_offset + 1, false, false);
        let cursor = app.editors[idx].cursor;
        assert!((10..14).contains(&cursor), "cursor should land within row 1's own char range, got {cursor}");
        assert_eq!(cursor, 11, "column 1 into \"CCCC\" (which starts at char 10) is char 11");
    }

    #[test]
    fn word_wrap_ensure_cursor_visible_scrolls_so_the_cursors_own_wrapped_row_is_shown() {
        let dir = workspace();
        // 3 lines, each of which wraps to 2 rows at width 9 (6 visual rows
        // total) — same shape as the click test above.
        ...
        // Move the cursor to the last line's second wrapped row ("CCCC").
        app.editors[idx].cursor = app.editors[idx].rope.line_to_char(2) + 11;

        // A short viewport (4 rows) that can't show all 6 visual rows at
        // once, at the gutter(4) + content(9) width used above.
        app.ensure_cursor_visible(4, 4 + 9);

        let b = &app.editors[idx];
        let pos = b
            .wrap_position(b.scroll_row, 2, 11, 9)
            .expect("cursor's line is at or after the new scroll_row");
        assert!(pos.0 < 4, "the cursor's own wrapped row must be within the visible viewport, got row offset {}", pos.0);
    }

    #[test]
    fn word_wrap_scrollbar_reflects_wrapped_row_count_not_just_line_count() {
        let dir = workspace();
        // One line that, unwrapped, easily fits a short viewport (so the
        // scrollbar would normally have nothing to show) — but wrapped to a
        // narrow width it produces enough rows to need a thumb.
        let content = "word ".repeat(40); // 200 chars, one line
        fs::write(dir.path().join("long.txt"), content).unwrap();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.open_path(&dir.path().join("long.txt"));
        let idx = app.active_editor.unwrap();

        let (total_off, _) = app.editors[idx].wrap_scroll_metrics(0, usize::MAX);
        assert_eq!(total_off, 1, "a single line is one row without wrap");

        app.word_wrap = true;
        let (total_on, _) = app.editors[idx].wrap_scroll_metrics(0, 20);
        assert!(total_on > 1, "the same line wrapped to a narrow width spans multiple rows");
    }
    ...
    fn search_files_auto_search_finds_matches_across_files() {
        ...
        assert!(app.poll_pending_search(), "poll should report that a search job just started");
    ...
    fn terminal_picker_filters_and_jumps_to_a_match() {
        // Fixed-name root, for the same reason as
        // `terminal_picker_backspace_widens_the_filter_again`: the labels this
        // filters contain the folder name, and a random tempdir name holding
        // the filtered digit makes unrelated terminals match.
        let dir = workspace();
    ...
    fn terminal_picker_backspace_widens_the_filter_again() {
        // Rooted at a fixed-name subfolder, not the tempdir itself: the picker
        // fuzzy-matches each terminal's "folder · command" label, and
        // `tempdir()` names are random — one containing the digit this test
        // filters on made every terminal match, failing roughly one run in
        // three.
        let dir = workspace();
        let root = dir.path().join("ws");
        std::fs::create_dir_all(&root).unwrap();
    ...
    #[test]
    fn settings_dialog_has_font_fps_size_wrap_and_colour_sections() {
        let dir = tempfile::tempdir().unwrap();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.open_settings();
        ...
        app.settings_move_focus(1);
        assert_eq!(app.settings_focus, 3); // word wrap
        app.settings_move_focus(1);
        assert_eq!(app.settings_focus, 4); // colour
        app.settings_move_focus(1);
        assert_eq!(app.settings_focus, 0, "wraps back to font (only five sections)");
    }
    ...
    fn settings_move_focus_negative_delta_goes_backward() {
        ...
        app.settings_move_focus(1);
        assert_eq!(app.settings_focus, 4, "Down moves forward again");
    }
    ...
    fn settings_adjusts_font_and_accent() {
        ...
        app.settings_move_focus(1);
        app.settings_move_focus(1);
        assert_eq!(app.settings_focus, 4);
    ...
    #[test]
    fn settings_adjusts_word_wrap_toggle_and_persists() {
        let dir = workspace();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        ...
        app.settings_move_focus(3);
        assert_eq!(app.settings_focus, 3);

        app.settings_adjust(1);
        assert!(app.word_wrap);
        app.settings_adjust(-1); // either direction just flips it
        assert!(!app.word_wrap);
        app.settings_adjust(1);
        assert!(app.word_wrap);

        let saved = crate::config::Config::load_with(Some(&config_path), dir.path());
        assert!(saved.word_wrap(), "toggled without closing the dialog, still reached disk");
    }
    ...
    #[test]
    fn settings_adjusts_dialog_size_within_range() {
        let dir = workspace();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.config_path = Some(dir.path().join("scratch-config.toml"));
        ...
        app.settings_adjust(-60); // clamps at the floor
        assert_eq!(app.dialog_size_pct, crate::settings::number_range(crate::settings::Id::DialogSize).0);
        app.settings_adjust(60); // clamps at the ceiling
        assert_eq!(app.dialog_size_pct, crate::settings::number_range(crate::settings::Id::DialogSize).1);
    }
    ...
    #[test]
    fn pasting_from_history_inserts_into_the_editor() {
        let dir = workspace();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.open_path(&dir.path().join("src/main.rs"));
        app.set_clipboard("OLDER".into());
        app.set_clipboard("NEWER".into());
        app.open_clipboard_dialog();
        app.clip_move(1); // onto the older entry
        app.clip_paste_selected();

        assert!(app.active_buffer().unwrap().rope.to_string().contains("OLDER"));
        assert_eq!(app.top_dialog(), None, "the dialog closes after pasting");
    }

    /// Pasting from the history only pastes. It doesn't re-copy what it
    /// pasted, so the system clipboard is untouched and the history keeps the
    /// order it had — picking an old entry doesn't shuffle it to the top.
    #[test]
    fn pasting_from_history_leaves_the_clipboard_and_the_order_alone() {
        let dir = workspace();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.open_path(&dir.path().join("src/main.rs"));
        for t in ["a", "b", "c"] {
            app.set_clipboard(t.to_string());
        }
        app.open_clipboard_dialog();
        app.clip_move(2); // the oldest, "a"
        app.clip_paste_selected();

        assert_eq!(app.clipboard_text, "c", "the clipboard still holds the last copy");
        let order: Vec<&str> = app.clip_ring.iter().map(|e| e.text.as_str()).collect();
        assert_eq!(order, vec!["c", "b", "a"], "pasting didn't reorder the history");
    }
    ...
    fn selecting_with_the_mouse_does_not_break_scroll_lock() {
        ...
        // Wait for the *last* line rather than draining for a fixed few
        // seconds: on a machine running the rest of the suite in parallel, the
        // shell hadn't always printed anything yet when the guess ran out, and
        // the assertions below then compared a half-filled screen.
        assert!(
            pump_until(&mut app, |t| screen_has(t, "LINE-60")),
            "the shell should have printed all 60 lines"
        );
        ...
        // Time-boxed rather than waiting for NOISE-80: the whole point is that
        // it never becomes visible, so there's nothing on screen to wait for.
        // Draining less output than expected can only make this weaker, never
        // wrong — the assertion is that the view did not move.
    ...
    /// Dropped onto a terminal, a file is a path you were about to type: it
    /// arrives at the shell as quoted text and opens no tab.
    #[test]
    fn dropping_on_a_terminal_types_the_path_instead_of_opening_it() {
        let dir = workspace();
        let f = dir.path().join("src/main.rs");
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.toggle_terminal_modal();
        let i = app.active_terminal;
        let _ = app.terminals[i].take_sent(); // drop the shell's startup traffic

        app.drop_path(&f, 0, 0);

        let sent = String::from_utf8_lossy(&app.terminals[i].take_sent()).into_owned();
        assert!(sent.contains("main.rs"), "the path reached the shell: {sent:?}");
        assert!(sent.ends_with(' '), "a trailing space so you can keep typing: {sent:?}");
        assert!(!sent.contains('\r'), "typed, not run: {sent:?}");
        assert!(app.editors.is_empty(), "and no tab was opened");
    }

    /// A path with a space stays one argument.
    #[test]
    fn a_dropped_path_with_spaces_is_quoted_for_the_shell() {
        let dir = workspace();
        let sub = dir.path().join("My Docs");
        std::fs::create_dir(&sub).unwrap();
        let f = sub.join("notes.txt");
        ...
        app.drop_path(&f, 0, 0);
        let sent = String::from_utf8_lossy(&app.terminals[i].take_sent()).into_owned();
        assert!(sent.contains("My\\ Docs"), "the space is escaped: {sent:?}");
    }

    /// Dropped anywhere else, the same file opens as a tab.
    #[test]
    fn dropping_outside_the_terminal_opens_a_tab() {
        let dir = workspace();
        let f = dir.path().join("src/main.rs");
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        assert!(app.top_dialog().is_none(), "no terminal open");

        app.drop_path(&f, 0, 0);
        assert_eq!(app.editors.len(), 1, "the file opened as a tab");
        assert_eq!(app.editors[0].path.as_deref(), Some(f.as_path()));
    }

    /// A dialog stacked over the terminal owns the drop, so it opens a tab.
    #[test]
    fn a_dialog_over_the_terminal_takes_the_drop_as_a_file() {
        let dir = workspace();
        let f = dir.path().join("src/main.rs");
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.toggle_terminal_modal();
        app.open_help(); // now on top of the terminal
        let i = app.active_terminal;
        let _ = app.terminals[i].take_sent();

        app.drop_path(&f, 0, 0);
        assert_eq!(app.editors.len(), 1, "the file opened as a tab");
        assert!(app.terminals[i].take_sent().is_empty(), "nothing reached the shell");
    }

    /// An unsupported file says so rather than opening an empty or broken tab.
    #[test]
    fn dropping_something_unopenable_says_so() {
        let dir = workspace();
        let f = dir.path().join("mystery"); // no extension, binary content
        std::fs::write(&f, [0x00, 0x01, 0x02, 0x03]).unwrap();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();

        app.drop_path(&f, 0, 0);
        assert!(app.editors.is_empty(), "no tab opened");
        let toast = app.toast.as_ref().expect("the user is told why");
        assert!(toast.message.contains("not a text file"), "{}", toast.message);
    }

    /// Dropping a folder is refused in words, not by a silent failure.
    #[test]
    fn dropping_a_folder_explains_itself() {
        let dir = workspace();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.drop_path(&dir.path().join("src"), 0, 0);
        assert!(app.editors.is_empty());
        let toast = app.toast.as_ref().expect("the user is told why");
        assert!(toast.message.contains("folder"), "{}", toast.message);
    }

    /// Several files copied together paste as a valid argument list.
    #[test]
    fn copied_files_join_as_shell_arguments() {
        let files = [
            std::path::PathBuf::from("/tmp/a.txt"),
            std::path::PathBuf::from("/tmp/My Docs/b.txt"),
        ];
        assert_eq!(App::paths_as_shell_args(&files), "/tmp/a.txt /tmp/My\\ Docs/b.txt");
    }


    // ---- clipboard history: multi-select --------------------------------

    /// An app with three things copied, history newest-first: c, b, a.
    fn clip_app() -> (tempfile::TempDir, App) {
        let dir = workspace();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.open_path(&dir.path().join("src/main.rs"));
        for t in ["a", "b", "c"] {
    ...
    fn buffer_text(app: &App) -> String {
        app.editors.first().map(|b| b.rope.to_string()).unwrap_or_default()
    }

    /// Ticking nothing keeps the old behaviour exactly: Enter pastes the row
    /// under the cursor.
    #[test]
    fn with_nothing_ticked_enter_pastes_the_highlighted_entry() {
        let (_d, mut app) = clip_app();
        app.clip_move(1); // onto "b"
        app.clip_paste_selected();
        assert!(buffer_text(&app).contains('b'));
        assert!(!buffer_text(&app).contains('c'), "only the one row");
    }

    /// Several ticked entries paste together, one per line, in list order.
    #[test]
    fn ticked_entries_paste_together_newline_separated() {
        let (_d, mut app) = clip_app();
        app.clip_toggle(); // c (newest, top)
        app.clip_move(1);
        app.clip_toggle(); // b
        app.clip_move(1);
        app.clip_toggle(); // a
        assert_eq!(app.clip_checked_count(), 3);

        app.clip_paste_selected();
        let text = buffer_text(&app);
        assert!(text.contains("c\nb\na"), "listed order, newline separated: {text:?}");
    }

    /// Ticking is a toggle, and the cursor row need not be one of them — the
    /// ticks win over wherever the cursor happens to be sitting.
    #[test]
    fn ticks_win_over_the_cursor_and_untick_again() {
        let (_d, mut app) = clip_app();
        app.clip_move(2); // cursor on "a"
        app.clip_toggle();
        app.clip_move(-2); // cursor back on "c", but "a" is ticked
        assert_eq!(app.clip_checked_count(), 1);

        app.clip_toggle(); // tick "c" too
        app.clip_toggle(); // and untick it again
        assert_eq!(app.clip_checked_count(), 1, "toggling twice is a no-op");

        app.clip_paste_selected();
        let text = buffer_text(&app);
        assert!(text.contains('a'), "the ticked row pasted: {text:?}");
        assert!(!text.contains('c'), "not the cursor row: {text:?}");
    }


    /// Ticks are a choice made in the moment — reopening the dialog starts clean.
    #[test]
    fn ticks_do_not_survive_closing_the_dialog() {
        let (_d, mut app) = clip_app();
        app.clip_toggle();
        assert_eq!(app.clip_checked_count(), 1);
        app.close_top_dialog();
        app.open_clipboard_dialog();
        assert_eq!(app.clip_checked_count(), 0, "a fresh open has nothing ticked");
    }


    /// Clearing the history clears the ticks with it.
    #[test]
    fn clearing_the_history_clears_the_ticks() {
        let (_d, mut app) = clip_app();
        app.clip_toggle();
        app.clip_clear();
        assert_eq!(app.clip_checked_count(), 0);
        assert!(app.clip_checked.is_empty());
    }
