...
pub fn handle_key(app: &mut App, key: KeyEvent) {
    ...
    macro_rules! on {
        ($id:expr) => {
            keys::hit($id, &key, ctrl, alt, shift)
        };
    }
    if on!(id::TERMINAL_TOGGLE) {
...
fn handle_find(app: &mut App, key: KeyEvent, ctrl: bool, alt: bool, shift: bool) {
    if keys::hit(id::FIND_REPLACE_TOGGLE, &key, ctrl, alt, shift) {
        app.find_toggle_replace();
        return;
    }
    if keys::hit(id::FIND_SWITCH_FIELD, &key, ctrl, alt, shift) && app.find.replace_active {
    ...
    match key.code {
        ...
        // Everything else — motion, selection, the deletes, the clipboard and
        // typing — is the shared one-line field behaviour.
        _ => {
            app.field_key(FieldId::FindQuery, key, ctrl, alt, shift);
        }
    }
}
...
fn handle_replace_field(app: &mut App, key: KeyEvent, ctrl: bool, alt: bool, shift: bool) {
    match key.code {
        ...
        // Everything else is the shared one-line field behaviour.
        _ => {
            app.field_key(FieldId::FindReplace, key, ctrl, alt, shift);
        }
    }
}
...
fn handle_prompt(app: &mut App, key: KeyEvent, ctrl: bool) {
    ...
    let (ctrl_held, alt, shift) = shortcut_mods(&key);
    let _ = ctrl_held;
    match key.code {
        ...
        _ => {
            app.field_key(FieldId::Prompt, key, ctrl, alt, shift);
        }
    }
}
...
fn handle_file_dialog(app: &mut App, key: KeyEvent, ctrl: bool, alt: bool, shift: bool) {
    match key.code {
        ...
        // Plain Left/Right drive the tree, so they're claimed here before the
        // field sees them; Option+Arrow still moves the query cursor by word.
        KeyCode::Right if !alt && !ctrl => app.dialog_tree_expand(),
        KeyCode::Left if !alt && !ctrl => app.dialog_tree_collapse(),
        ...
        _ if keys::hit(id::FD_COPY_PATH, &key, ctrl, alt, shift) => app.copy_dialog_path(false),
        _ if keys::hit(id::FD_COPY_REL, &key, ctrl, alt, shift) => app.copy_dialog_path(true),
        ...
        _ if keys::hit(id::FD_JUNK, &key, ctrl, alt, shift) => app.dialog_toggle_junk(),
        // New folder is Shift+Option+N (New file stays ⌘N / Ctrl+N).
        _ if keys::hit(id::FD_NEW_FOLDER, &key, ctrl, alt, shift) => app.dialog_new_folder(),
        _ if keys::hit(id::FD_NEW_FILE, &key, ctrl, alt, shift) => app.dialog_new_file(),
        _ if keys::hit(id::FD_DELETE, &key, ctrl, alt, shift) => app.dialog_delete(),
        _ if keys::hit(id::FD_RENAME, &key, ctrl, alt, shift) => app.dialog_rename(),
        // ⌥R reveals the selected file/folder in Finder — ⌥ (not ⌘R, which
        // renames) avoids a collision within this dialog.
        _ if keys::hit(id::FD_REVEAL, &key, ctrl, alt, shift) => app.dialog_reveal_in_finder(),
        // Plain Backspace also steps back out of a scoped folder, so it stays
        // here; Option+Backspace is an ordinary delete-word and falls through.
        KeyCode::Backspace if !alt => app.dialog_backspace(),
        // Everything else is the shared one-line field behaviour.
        _ => {
            app.field_key(FieldId::FileQuery, key, ctrl, alt, shift);
        }
    }
}
...
fn handle_search_files_dialog(app: &mut App, key: KeyEvent, ctrl: bool, alt: bool, shift: bool) {
    match key.code {
        ...
        // Everything else is the shared one-line field behaviour.
        _ => {
            app.field_key(FieldId::SearchQuery, key, ctrl, alt, shift);
        }
    }
}
...
fn handle_todos_dialog(app: &mut App, key: KeyEvent, ctrl: bool, alt: bool, shift: bool) {
    match key.code {
        ...
        _ if keys::hit(id::TODO_EDIT, &key, ctrl, alt, shift) => app.todo_edit_as_file(),
        _ if keys::hit(id::TODO_CLEAR, &key, ctrl, alt, shift) => app.todo_clear_done(),
        _ if keys::hit(id::TODO_DELETE, &key, ctrl, alt, shift) => app.todo_remove(),
        // A multi-line paste becomes one task per line — see `App::todo_paste`.
        _ if keys::hit(id::TODO_PASTE, &key, ctrl, alt, shift) => app.todo_paste(),
        // Everything else is the shared one-line field behaviour.
        _ => {
            app.field_key(FieldId::TodoInput, key, ctrl, alt, shift);
        }
    }
}
...
fn handle_clipboard_dialog(app: &mut App, key: KeyEvent, ctrl: bool) {
    match key.code {
        ...
        // Space ticks a row; Enter then pastes every ticked one, newline
        // separated. Same gesture as the recent-folders list.
        KeyCode::Char(' ') => app.clip_toggle(),
        _ if keys::hit(id::CLIP_CLEAR, &key, ctrl, false, false) => app.clip_clear(),
        _ => {}
    }
}
...
fn handle_editor_context(app: &mut App, key: KeyEvent, ctrl: bool, alt: bool, shift: bool) {
    ...
    if ctrl {
        match key.code {
            ...
            _ if keys::hit(id::MULTI_CURSOR, &key, ctrl, alt, shift) => app.add_next_occurrence(),
            ...
            _ if keys::hit(id::EDITOR_SAVE_ALL, &key, ctrl, alt, shift) => app.save_all(),
            _ if keys::hit(id::EDITOR_SAVE, &key, ctrl, alt, shift) => app.save_active(),
            _ if keys::hit(id::EDITOR_CLOSE_ALL, &key, ctrl, alt, shift) => app.close_all(),
            _ if keys::hit(id::EDITOR_CLOSE, &key, ctrl, alt, shift) => app.close_tab(),
            // Find a word in the current file (case-insensitive).
            _ if keys::hit(id::FIND_OPEN, &key, ctrl, alt, shift) => app.find_open(),
            // Reopen the most-recently-closed tab.
            _ if keys::hit(id::EDITOR_REOPEN, &key, ctrl, alt, shift) => app.reopen_closed_tab(),
            // Toggle the split / grid view (tile all open files).
            _ if keys::hit(id::EDITOR_SPLIT, &key, ctrl, alt, shift) => app.toggle_editor_grid(),
            _ if keys::hit(id::EDITOR_SELECT_ALL, &key, ctrl, alt, shift) => app.select_all(),
            // Ctrl+Shift+C/R copy the open file's full / relative path.
            _ if keys::hit(id::EDITOR_COPY_PATH, &key, ctrl, alt, shift) => app.copy_active_path(false),
            _ if keys::hit(id::EDITOR_COPY_REL, &key, ctrl, alt, shift) => app.copy_active_path(true),
            _ if keys::hit(id::EDITOR_COPY, &key, ctrl, alt, shift) => app.copy_selection(),
            _ if keys::hit(id::EDITOR_CUT, &key, ctrl, alt, shift) => app.cut_selection(),
            _ if keys::hit(id::EDITOR_PASTE, &key, ctrl, alt, shift) => app.paste(),
            // Undo / redo. Ctrl+Z undo, Ctrl+Shift+Z or Ctrl+Y redo.
            _ if keys::hit(id::EDITOR_REDO, &key, ctrl, alt, shift) => app.redo(),
            _ if keys::hit(id::EDITOR_UNDO, &key, ctrl, alt, shift) => app.undo(),
            KeyCode::Char('y') | KeyCode::Char('Y') => app.redo(),
            // Ctrl+Shift+,/. (i.e. < / >) move the active tab.
            _ if keys::hit(id::TAB_MOVE_LEFT, &key, ctrl, alt, shift) => app.move_tab_left(),
            _ if keys::hit(id::TAB_MOVE_RIGHT, &key, ctrl, alt, shift) => app.move_tab_right(),
            KeyCode::Char(',') if shift => app.move_tab_left(),
            KeyCode::Char('.') if shift => app.move_tab_right(),
            // Ctrl+Tab next tab, Ctrl+Shift+Tab previous.
            _ if keys::hit(id::TAB_PREV, &key, ctrl, alt, shift) => app.prev_tab(),
            _ if keys::hit(id::TAB_NEXT, &key, ctrl, alt, shift) => app.next_tab(),
...
fn handle_terminal_modal(app: &mut App, key: KeyEvent, alt: bool) {
    ...
    if keys::hit(id::TERM_PICKER, &key, ctrl, alt, shift) {
        app.open_terminal_picker();
        return;
    }
    if keys::hit(id::TERM_JUMP, &key, ctrl, alt, shift) {
    ...
    if alt {
        match key.code {
            ...
            _ if keys::hit(id::TERMINAL_TOGGLE, &key, ctrl, alt, shift) => {
                app.toggle_terminal_modal();
                return;
            }
            _ if keys::hit(id::TERM_NEW, &key, ctrl, alt, shift) => {
                app.new_terminal();
                return;
            }
            _ if keys::hit(id::TERM_CLOSE, &key, ctrl, alt, shift) => {
                app.close_terminal();
                return;
            }
            _ if keys::hit(id::TERM_GRID, &key, ctrl, alt, shift) => {
