...
fn render_terminal_modal(f: &mut Frame, area: Rect, app: &mut App, _below: usize) {
    ...
    let actions: Vec<(String, String)> = if app.terminal_copy_mode() {
        ...
        vec![
            keys::hint(id::CM_MOVE),
            keys::hint(id::CM_MARK),
            keys::hint(id::CM_WORD),
            // The one row that names another binding: copy-and-exit also
            // answers to whatever the terminal's copy chord is.
            keys::hint_chord(
                id::CM_COPY,
                format!("{} / {}", keys::chord(id::CM_COPY), keys::chord(id::TERM_COPY)),
            ),
            keys::hint(id::CM_SCROLL),
            keys::hint(id::CM_EXIT),
        ]
    } else {
        ...
        // keeps its own literal chords to keep behaving like a real terminal
        // app. Quit is the one exception: it's the global ⌘Q, not a
        // terminal-local binding.
        let mut actions = vec![keys::hint(id::TERM_NEW), keys::hint(id::TERM_CLOSE)];
        if app.terminals.len() > 1 {
            // Switching / moving mirror the editor tabs (Ctrl+Tab, Ctrl+Shift+,/.).
            actions.push(keys::hint(id::TERM_NEXT));
            actions.push(keys::hint(id::TERM_PREV));
            actions.push(keys::hint(id::TERM_MOVE));
            actions.push(keys::hint_as(
                id::TERM_GRID,
                if app.terminal_grid { "Tabs" } else { "Grid" },
            ));
            actions.push(keys::hint(id::TERM_PICKER));
            actions.push(keys::hint(id::TERM_JUMP));
        }
        actions.push(keys::hint_as(id::TERMINAL_TOGGLE, "Hide"));
        actions.push(keys::hint(id::TERM_WORD));
        actions.push(keys::hint(id::TERM_DEL));
        actions.push(keys::hint(id::TERM_MARK));
        actions.push(keys::hint(id::TERM_COPY_MODE));
        actions.push(keys::hint(id::TERM_SCROLL));
        actions.push(keys::hint(id::TERM_COPY));
        actions.push(keys::hint(id::TERM_PASTE));
        actions.push(keys::hint(id::QUIT));
        actions
    };

    let refs = keys::as_refs(&actions);
...
fn render_terminal_picker(f: &mut Frame, area: Rect, app: &App, below: usize) {
    ...
    // This was the one dialog that advertised nothing at all — it handled ↑↓,
    // Enter, Esc and typing, and told you about none of them.
    render_divider(f, parts[3], t);
    let foot = keys::hints(&[id::PICK_NAV, id::PICK_OPEN, id::PICK_TYPE, id::PICK_CLOSE]);
    let foot = keys::as_refs(&foot);
    f.render_widget(
...
fn render_search_files(f: &mut Frame, area: Rect, app: &mut App, below: usize) {
    ...
    let actions = keys::hints(&[id::SF_NAV, id::SF_OPEN, id::SF_CLOSE]);
    let refs = keys::as_refs(&actions);
    f.render_widget(
...
fn render_blank(f: &mut Frame, area: Rect, app: &App) {
    ...
    if app.has_folder() {
        lines.push(hint("Open files", keys::chord(id::FILES_OPEN)));
        lines.push(hint("Search in files", keys::chord(id::SEARCH_FILES_OPEN)));
        lines.push(hint("Open a folder", keys::chord(id::FOLDER_OPEN)));
        lines.push(hint("Recent folders", keys::chord(id::RECENT_OPEN)));
        lines.push(hint("New terminal", keys::chord(id::TERMINAL_TOGGLE)));
    } else {
    ...
    lines.push(hint("Settings", keys::chord(id::SETTINGS_OPEN)));
    lines.push(hint("Shortcuts", keys::chord(id::HELP)));
    lines.push(hint("Quit", keys::chord(id::QUIT)));
...
fn render_footer(f: &mut Frame, area: Rect, app: &App) {
    ...
    let mut hints: Vec<(String, String)> = keys::hints(&[
        id::EDITOR_SAVE,
        id::FIND_OPEN,
        id::EDITOR_UNDO,
        id::EDITOR_REDO,
    ...
    if b.selection().is_some() {
        hints.push(keys::hint(id::EDITOR_COPY));
        hints.push(keys::hint(id::EDITOR_CUT));
    } else {
        hints.push(keys::hint(id::EDITOR_SELECT_ALL));
    }
    hints.extend(keys::hints(&[
        id::EDITOR_PASTE,
        id::EDITOR_SAVE_ALL,
        id::EDITOR_CLOSE,
        id::EDITOR_CLOSE_ALL,
    ...
    hints.push(keys::hint_as(
        id::EDITOR_SPLIT,
        if app.editor_grid { "Unsplit" } else { "Split" },
    ));
    if app.editors.len() > 1 {
        hints.push(keys::hint(id::TAB_NEXT));
        hints.push(keys::hint(id::TAB_MOVE_HINT));
    }
    // Multi-cursor: the add-next-match chord, and — while several carets are
    // live — how to add a column caret and how to collapse back to one.
    if b.has_extra_carets() {
        hints.push(keys::hint(id::CARET_ABOVE));
        hints.push(keys::hint(id::CARET_COLLAPSE));
    } else {
        hints.push(keys::hint(id::MULTI_CURSOR));
    }
    hints.extend(keys::hints(&[
        id::EDITOR_COPY_PATH,
        id::FILES_OPEN,
        id::SEARCH_FILES_OPEN,
        id::TERMINAL_TOGGLE,
    ]));
    hints.push(keys::hint_as(
        id::WORD_WRAP,
        if app.word_wrap { "Unwrap" } else { "Wrap" },
    ));
    hints.extend(keys::hints(&[id::SETTINGS_OPEN, id::HELP, id::QUIT]));
...
fn render_find_bar(f: &mut Frame, area: Rect, app: &App) {
    ...
    if inner.height > hints_row {
        ...
        let dim = Style::default().fg(t.fg_dim);
        // Built from the same table as everything else — the old hand-written
        // version showed the replace toggle as "^R", a third notation for a
        // chord written as ⌘R elsewhere and bound to Ctrl-or-⌘.
        // Tab is labelled by where it takes you, so it reads as a direction
        // rather than a mode name.
        let row = if app.find.replace_focus {
            vec![
                keys::hint(id::FIND_REPLACE_ONE),
                keys::hint(id::FIND_REPLACE_ALL),
                keys::hint_as(id::FIND_SWITCH_FIELD, "find"),
                keys::hint(id::FIND_CLOSE),
            ]
        } else if app.find.replace_active {
            vec![
                keys::hint(id::FIND_NEXT),
                keys::hint_as(id::FIND_SWITCH_FIELD, "replace"),
                keys::hint(id::FIND_CLOSE),
            ]
        } else {
            vec![
                keys::hint(id::FIND_NEXT),
                keys::hint(id::FIND_PREV),
                keys::hint(id::FIND_REPLACE_TOGGLE),
                keys::hint(id::FIND_CLOSE),
            ]
        };
        let mut spans: Vec<TSpan> = Vec::new();
        for (i, (chord, label)) in row.into_iter().enumerate() {
            if i > 0 {
                spans.push(TSpan::styled("  ", dim));
            }
            spans.push(TSpan::styled(chord, acc));
            spans.push(TSpan::styled(format!(" {label}"), dim));
        }
...
fn render_todos(f: &mut Frame, area: Rect, app: &App, below: usize) {
    ...
    if app.todo_input.is_empty() {
        ...
        f.render_widget(
            Paragraph::new(Line::from(TSpan::styled(
                format!(
                    "type to add a task \u{00b7} {} pastes a list",
                    keys::chord(id::TODO_PASTE)
                ),
...
fn render_clipboard(f: &mut Frame, area: Rect, app: &App, below: usize) {
    ...
    for (row, entry) in app.clip_ring.iter().enumerate().skip(start).take(list_h) {
        ...
        let checked = app.clip_checked.get(row).copied().unwrap_or(false);
        let base = if on_cursor { t.selection(true) } else { Style::default().fg(t.fg) };
        // Same checkbox column as the recent-folders list, so ticking rows
        // reads the same way in both places.
        let check = if checked { "\u{2611}" } else { "\u{2610}" }; // ☑ / ☐
        let mut spans = vec![
            TSpan::styled(
                format!(" {check} "),
                if checked && !on_cursor { Style::default().fg(t.accent) } else { base },
            ),
            TSpan::styled(entry.label.clone(), base),
        ];
    ...
    // The Enter hint counts what it's about to do, so ticking several rows and
    // then pressing it isn't a leap of faith.
    let ticked = app.clip_checked_count();
    let paste_label = if ticked > 0 { format!("Paste {ticked}") } else { "Paste".to_string() };
    let foot = vec![
        keys::hint_as(id::CLIP_PASTE, &paste_label),
        keys::hint(id::CLIP_NAV),
        keys::hint(id::CLIP_TOGGLE),
        keys::hint(id::CLIP_CLEAR),
        keys::hint(id::CLIP_CLOSE),
    ];
    let foot = keys::as_refs(&foot);
    f.render_widget(
...
fn render_recent(f: &mut Frame, area: Rect, app: &App, below: usize) {
    ...
    let foot = vec![
        keys::hint(id::REC_NAV),
        keys::hint(id::REC_TOGGLE),
        keys::hint_as(id::REC_OPEN, &open_label),
        keys::hint(id::REC_REMOVE),
        keys::hint(id::FOLDER_OPEN),
        keys::hint(id::REC_CLOSE),
    ];
    let foot = keys::as_refs(&foot);
    f.render_widget(
        Paragraph::new(hint_row(&foot, parts[4].width, t)).style(Style::default().bg(t.bg_dark)),
...
fn render_settings(f: &mut Frame, area: Rect, app: &App, below: usize) {
    ...
    // Every row is drawn from the settings table: its label, a control chosen
    // by its `Kind`, and its hint line. Adding a preference there makes it
    // appear here with no edit to this function.
    let dim = Style::default().fg(t.fg_dim);
    let label = |text: &str, focused: bool| {
        ...
        Line::from(vec![
            TSpan::styled(marker, style),
            TSpan::styled(text.to_string(), style),
        ])
    };

    let mut lines: Vec<Line> = vec![Line::from("")];

    for (i, row) in st::SETTINGS.iter().enumerate() {
        let focused = app.settings_focus == i;
        // The value reads in the accent when its row has the keyboard.
        let value_style = Style::default()
            .fg(if focused { t.accent } else { t.fg })
            .add_modifier(Modifier::BOLD);
        lines.push(label(row.label, focused));

        match (row.kind, app.setting_value(row.id)) {
            // `-  N  +`, with the hint sharing the line.
            (st::Kind::Number { suffix, .. }, st::Value::Num(n)) => {
                lines.push(Line::from(vec![
                    TSpan::raw("    "),
                    TSpan::styled("-", dim),
                    TSpan::styled(format!("  {n}{suffix}  "), value_style),
                    TSpan::styled("+", dim),
                    TSpan::styled(format!("     {}", row.hint), dim),
                ]));
            }
            // The whole list, the picked one underlined.
            (st::Kind::Choice { options, .. }, st::Value::Num(current)) => {
                let mut spans: Vec<TSpan> = vec![TSpan::raw("    ")];
                for (n, &opt) in options.iter().enumerate() {
                    if n > 0 {
                        spans.push(TSpan::raw("  "));
                    }
                    let style = if opt == current {
                        value_style.add_modifier(Modifier::UNDERLINED)
                    } else {
                        dim
                    };
                    spans.push(TSpan::styled(opt.to_string(), style));
                }
                lines.push(Line::from(spans));
                lines.push(Line::from(vec![
                    TSpan::raw("    "),
                    TSpan::styled(row.hint.to_string(), dim),
                ]));
            }
            (st::Kind::Toggle { .. }, st::Value::Flag(on)) => {
                lines.push(Line::from(vec![
                    TSpan::raw("    "),
                    TSpan::styled(if on { "On" } else { "Off" }, value_style),
                    TSpan::styled(format!("     {}", row.hint), dim),
                ]));
            }
            // Swatches, with a tick on the picked one in whichever of black or
            // white reads against it.
            (st::Kind::Palette, st::Value::Pick(picked)) => {
                let mut swatches: Vec<TSpan> = vec![TSpan::raw("    ")];
                for (n, (_, (r, g, b))) in ACCENT_PALETTE.iter().enumerate() {
                    let color = Color::Rgb(*r, *g, *b);
                    if n == picked {
                        let check = if luminance(*r, *g, *b) > 140 { Color::Black } else { Color::White };
                ...
                lines.push(Line::from(vec![
                    TSpan::raw("    "),
                    TSpan::styled(
                        ACCENT_PALETTE[picked].0.to_string(),
                        Style::default().fg(t.accent).add_modifier(Modifier::BOLD),
                    ),
                ]));
            }
            // A kind and a value that don't pair up is a bug in `App::setting_value`,
            // not something the user should see a broken dialog for.
            _ => lines.push(Line::from(TSpan::styled("    (unavailable)", dim))),
        }
        if i + 1 < st::SETTINGS.len() {
            lines.push(Line::from(""));
        }
    }
    ...
    // Footer hints, justified across the full width.
    if inner.height >= 1 {
        // The sheet spells these out ("↑ ↓  Tab"); the footer has one row, so
        // it names the same keys in their tight form.
        let foot = vec![
            keys::hint_chord(id::SET_SECTION, "\u{2191}\u{2193}".into()),
            keys::hint_chord(id::SET_ADJUST, "\u{2190}\u{2192}".into()),
            keys::hint(id::SET_CLOSE),
            keys::hint(id::QUIT),
        ];
        let foot = keys::as_refs(&foot);
        if inner.height >= 2 {
...
fn render_file_dialog(f: &mut Frame, area: Rect, app: &mut App, below: usize) {
    ...
    // The file-management half is the same either way; only the navigation
    // keys at the front differ between browsing the tree and searching.
    let manage = [
        id::FD_NEW_FILE,
        id::FD_NEW_FOLDER,
        id::FD_RENAME,
    ...
    let mut actions: Vec<(String, String)> = if query_empty {
        let mut a = vec![
            keys::hint(id::FD_NAV),
            keys::hint(id::FD_OPEN),
            keys::hint(id::FD_EXPAND),
            keys::hint(id::FD_COLLAPSE),
            keys::hint_as(id::FD_TAB_IN, "In Folder"),
        ];
        ...
        if scope_rel.is_some() {
            a.push(keys::hint(id::FD_TAB_OUT));
        }
        a
    } else {
        vec![
            keys::hint(id::FD_NAV),
            keys::hint_as(id::FD_OPEN, &open_label),
            keys::hint(id::FD_TAB_IN),
        ]
    };
    actions.extend(keys::hints(&manage));
    if !query_empty {
        actions.push(keys::hint_as(id::FD_JUNK, junk_label));
    }
    actions.push(keys::hint(id::FD_CLOSE));
    actions.push(keys::hint(id::QUIT));
    let actions = keys::as_refs(&actions);
...
/// The F1 cheat-sheet's contents. One thin call: every row, its combo and its
/// section live in [`crate::keys`], the single table that also decides which
/// key event fires which action — so the sheet cannot drift from the bindings.
fn shortcut_groups(has_folder: bool) -> Vec<(&'static str, Vec<(String, String)>)> {
    keys::groups(has_folder)
}
...
fn render_help(f: &mut Frame, area: Rect, app: &mut App, below: usize) {
    ...
    render_divider(f, parts[3], &t);
    let foot = keys::hints(&[id::HELP_SCROLL, id::HELP_PAGE, id::HELP_CLOSE, id::QUIT]);
    let foot = keys::as_refs(&foot);
    f.render_widget(
...
mod tests {
    ...
    fn settings_dialog_shows_all_five_sections_and_survives_tiny_sizes() {
        ...
        for f in crate::settings::fps_options() {
    ...
    /// The drift guard. For each dialog, the chords its handler actually acts
    /// on — checked by pressing them and watching for the effect — must be
    /// printed on that dialog's own screen. This is the invariant that was
    /// broken in three places before `keys` existed: the quick-switcher
    /// advertised nothing at all, the to-do list never mentioned ⌘V, and the
    /// terminal footer named ⌘K for a binding that was ⌥K.
    fn press(app: &mut App, id: &str) {
        let s = keys::get(id);
        let mut m = KeyModifiers::NONE;
        match s.mods {
            keys::Mods::Cmd | keys::Mods::Ctrl => m = KeyModifiers::CONTROL,
            keys::Mods::CmdShift | keys::Mods::CtrlShift | keys::Mods::SuperShift => {
                m = KeyModifiers::CONTROL | KeyModifiers::SHIFT
            }
            keys::Mods::Alt => m = KeyModifiers::ALT,
            keys::Mods::AltShift => m = KeyModifiers::ALT | KeyModifiers::SHIFT,
            keys::Mods::Shift => m = KeyModifiers::SHIFT,
            keys::Mods::Plain | keys::Mods::Text => {}
        }
        let code = match s.key {
            "Tab" => KeyCode::Tab,
            "Esc" => KeyCode::Esc,
            "Space" => KeyCode::Char(' '),
            "F1" => KeyCode::F(1),
            "1-9" => KeyCode::Char('1'),
            "\u{21b5}" => KeyCode::Enter,
            "\u{232b}" => KeyCode::Backspace,
            "\u{2190}" => KeyCode::Left,
            "\u{2192}" => KeyCode::Right,
            "\u{2191}" => KeyCode::Up,
            "\u{2193}" => KeyCode::Down,
            k => KeyCode::Char(k.chars().next().unwrap().to_ascii_lowercase()),
        };
        crate::input::handle_key(app, KeyEvent::new(code, m));
    }

    /// Every chord in `ids` is handled here *and* named here — chord and label
    /// on one line, which is what a hint row is. Matching the chord alone
    /// against the whole screen proves nothing: the editor's status bar behind
    /// the dialog advertises ⌘V too, just for a different action.
    fn assert_advertised(app: &mut App, ids: &[&str], what: &str) {
        let mut terminal = Terminal::new(TestBackend::new(120, 30)).unwrap();
        terminal.draw(|f| render(f, app)).unwrap();
        let rows = screen_rows(&terminal);
        for id in ids {
            let (chord, label) = keys::hint(id);
            assert!(
                rows.iter().any(|r| r.contains(&chord) && r.contains(&label)),
                "{what} handles {chord} ({id}) but no line on screen reads \"{chord} … {label}\"",
            );
        }
    }

    #[test]
    fn the_todo_list_advertises_every_key_it_handles() {
        let dir = workspace();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.open_todos_dialog();
        // ⌘V turning a pasted list into tasks is the one that was missing.
        assert_advertised(
            &mut app,
            &[id::TODO_PASTE, id::TODO_EDIT, id::TODO_DELETE, id::TODO_CLEAR, id::TODO_ADD],
            "the to-do list",
        );
    }

    #[test]
    fn the_clipboard_history_advertises_every_key_it_handles() {
        let dir = workspace();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.open_clipboard_dialog();
        assert_advertised(&mut app, &[id::CLIP_CLEAR, id::CLIP_PASTE], "clipboard history");
    }

    /// The quick-switcher used to render no hints whatsoever.
    #[test]
    fn the_terminal_quick_switcher_advertises_its_keys() {
        let dir = workspace();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.toggle_terminal_modal();
        app.new_terminal(); // the picker only opens with something to pick between
        app.open_terminal_picker();
        assert_advertised(
            &mut app,
            &[id::PICK_OPEN, id::PICK_CLOSE, id::PICK_NAV],
            "the terminal quick-switcher",
        );
    }

    /// The terminal footer named ⌘K and ⌘1-9 for bindings that are ⌥K and ⌥1-9,
    /// so the two hints it printed were both keys that did nothing.
    #[test]
    fn the_terminal_footer_names_the_chords_that_actually_work() {
        let dir = workspace();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();
        app.toggle_terminal_modal();
        app.new_terminal(); // the switch/jump hints only show with 2+ terminals

        let mut terminal = Terminal::new(TestBackend::new(160, 40)).unwrap();
        terminal.draw(|f| render(f, &mut app)).unwrap();
        let text = screen_text(&terminal);
        for id in [id::TERM_PICKER, id::TERM_JUMP, id::TERM_NEW, id::TERM_CLOSE] {
            assert!(
                text.contains(&keys::chord(id)),
                "the terminal footer is missing {}",
                keys::chord(id)
            );
        }
        // And the chords it used to promise are not on screen, because they
        // are not what fires: ⌘K / ⌘1-9 belong to nothing.
        assert!(!text.contains("\u{2318}K"), "the footer still promises \u{2318}K");
        assert!(!text.contains("\u{2318}1-9"), "the footer still promises \u{2318}1-9");
    }

    /// Pressing what the footer promises has to do something. Driving the keys
    /// through `handle_key` proves the hint and the binding agree, which is the
    /// half a rendering test alone can't see.
    #[test]
    fn the_advertised_chords_actually_fire() {
        let dir = workspace();
        let mut app = App::new(Some(dir.path().to_path_buf())).unwrap();

        // ⌥D opens the to-do list, as the welcome screen and F1 both claim.
        press(&mut app, id::TODOS_OPEN);
        assert!(matches!(app.top_dialog(), Some(Dialog::Todos)), "\u{2325}D opens the to-do list");
        press(&mut app, id::TODO_CLOSE);

        // ⌥V opens clipboard history.
        press(&mut app, id::CLIP_OPEN);
        assert!(matches!(app.top_dialog(), Some(Dialog::Clipboard)), "\u{2325}V opens clipboard history");
        press(&mut app, id::CLIP_CLOSE);

        // ⌥T opens the terminal, and ⌥K the quick-switcher inside it — the
        // pair the old footer got wrong.
        press(&mut app, id::TERMINAL_TOGGLE);
        assert!(matches!(app.top_dialog(), Some(Dialog::Terminal)), "\u{2325}T opens the terminal");
        app.new_terminal(); // the picker only opens with something to pick between
        press(&mut app, id::TERM_PICKER);
        assert!(
            matches!(app.top_dialog(), Some(Dialog::TerminalPicker)),
            "\u{2325}K opens the quick-switcher"
        );
    }

    /// Find-and-replace was reachable but undocumented: the sheet listed only
    /// next / previous / close, so the replace field had no way in.
    #[test]
    fn the_find_bar_advertises_replace() {
        let dir = workspace();
        ...
        app.open_path(&f);
        app.find_open();
        assert_advertised(&mut app, &[id::FIND_REPLACE_TOGGLE], "the find bar");

        // And pressing it opens the replace field.
        press(&mut app, id::FIND_REPLACE_TOGGLE);
        assert!(app.find.replace_active, "the advertised replace chord opens the field");
    }
