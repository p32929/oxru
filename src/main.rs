...
//!
//! # House rules, enforced by the compiler
//!
//! **No unsafe outside [`sys`].** Every FFI call Oxru makes is in that one
//! file behind a safe wrapper; `deny(unsafe_code)` here means a new `unsafe`
//! anywhere else fails the build rather than being noticed in review.
//!
//! **No `unwrap` / `expect` / `panic!` in shipped code.** A crash in an editor
//! loses the user's unsaved work, so every fallible path returns, falls back or
//! degrades instead. The lints below are `deny` for real builds and off under
//! `cfg(test)`, where an unwrap *is* the assertion and a panic is how a test
//! reports failure. Run `cargo clippy` to check.
...
fn relaunch_gui_detached(args: &[String]) -> Result<()> {
    ...
    cmd.args(args)
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    crate::sys::detach_from_terminal(&mut cmd);
    cmd.spawn()?;
    Ok(())
}
