...
/// Build the shared walk configuration used both by the sequential walk below
/// and by [`crate::search`]'s parallel (ripgrep-engine) project search — one
/// place deciding what counts as "a project file", so search and Quick Open
/// can never silently disagree about which files exist. With `respect_gitignore`
/// the project's `.gitignore`s (root + nested) and `.ignore` / `.git/info/exclude`
/// are honoured; without it, every file is yielded. Either way: dotfiles are
/// shown, the user's *global* and any *parent* repo's ignores are not consulted,
/// and VCS / tool internals plus the heavy `node_modules` / `target` dirs are
/// always pruned.
pub(crate) fn walk_builder(root: &Path, respect_gitignore: bool, include_junk: bool) -> ignore::WalkBuilder {
...
/// Walk `root` for files, sequentially. See [`walk_builder`] for the rules.
fn collect_walk(root: &Path, respect_gitignore: bool, include_junk: bool) -> Vec<PathBuf> {
    let wb = walk_builder(root, respect_gitignore, include_junk);
