...
/// Smooth caret motion: the primary caret slides to its new cell over a few
/// frames instead of teleporting.
///
/// Only the *primary* caret is eased. Secondary (multi-cursor) carets appear
/// and vanish in groups with no correspondence between one frame's set and the
/// next — animating those would mean guessing which old caret became which new
/// one, and guessing wrong looks like carets flying across the screen.
#[derive(Default)]
struct CaretAnim {
    /// Last drawn position of the primary caret, in physical pixels.
    at: Option<(f32, f32)>,
}

impl CaretAnim {
    /// Fraction of the remaining distance covered per frame. Exponential decay
    /// rather than a fixed duration: it needs no timer, is frame-rate tolerant
    /// (a slow frame simply covers more ground), and settles asymptotically so
    /// there's no overshoot to correct.
    const EASE: f32 = 0.45;

    /// Distance in pixels below which the caret just snaps. Without it the
    /// exponential tail leaves the caret a fraction of a pixel off its cell
    /// forever, which on a fractional-scale display shows up as a permanently
    /// blurred edge.
    const SNAP_PX: f32 = 0.75;

    /// This frame's caret positions in physical pixels, easing the primary one
    /// toward its target. Returns `(x, y, colour)` per caret.
    fn step(
        &mut self,
        carets: &[crate::app::GuiCaret],
        cell_w: f32,
        cell_h: f32,
    ) -> Vec<(f32, f32, ratatui::style::Color)> {
        let target = |c: &crate::app::GuiCaret| (c.col as f32 * cell_w, c.row as f32 * cell_h);

        let Some(primary) = carets.first() else {
            // No caret this frame (blink-off, or focus moved to a dialog).
            // Forget the position rather than easing from it later: when the
            // caret comes back somewhere else entirely, it should be there
            // already, not slide across the file.
            self.at = None;
            return Vec::new();
        };

        let (tx, ty) = target(primary);
        let (x, y) = match self.at {
            Some((px, py)) => {
                let (dx, dy) = (tx - px, ty - py);
                // Land instantly in two cases. A jump of more than a couple of
                // rows isn't a cursor movement you can follow — it's a scroll,
                // a file switch, or a click far away, and sliding across that
                // distance draws a streak through unrelated text. And once
                // there's less than a pixel to go, easing the rest of the way
                // is invisible motion that keeps requesting frames forever.
                let far = dy.abs() > cell_h * 2.5 || dx.abs() > cell_w * 40.0;
                let arrived = dx.abs() < Self::SNAP_PX && dy.abs() < Self::SNAP_PX;
                if far || arrived {
                    (tx, ty)
                } else {
                    (px + dx * Self::EASE, py + dy * Self::EASE)
                }
            }
            None => (tx, ty),
        };
        ...
        let mut out = vec![(x, y, primary.color)];
        out.extend(carets.iter().skip(1).map(|c| {
            let (cx, cy) = target(c);
            (cx, cy, c.color)
        }));
        out
    ...
    /// Whether the caret is still mid-slide, so the loop knows to schedule
    /// another frame even though nothing else changed.
    fn settling(&self, carets: &[crate::app::GuiCaret], cell_w: f32, cell_h: f32) -> bool {
        let (Some((px, py)), Some(c)) = (self.at, carets.first()) else {
            return false;
        };
        let (tx, ty) = (c.col as f32 * cell_w, c.row as f32 * cell_h);
        (tx - px).abs() >= Self::SNAP_PX || (ty - py).abs() >= Self::SNAP_PX
    }
}
...
/// The gap between unattended repaints, from the user's configured FPS —
/// nothing else. Whatever the Settings dialog says is what the window actually
/// composites at, in both directions.
///
/// This used to be `max(App::repaint_interval(), 250ms)`, a floor meant to stop
/// an idle window burning 60fps on a screen with nothing to show. The floor won
/// for every setting above 4fps, so 5, 10, 20, 30 and 60 all rendered at the
/// same 4fps and the readout sat at "4.0/60" no matter what was picked — the
/// control did nothing across almost its whole range.
///
/// `RENDER_TICK / 2` of slack matters at the top of the range: the tick loop
/// sleeps ~16ms and the 60fps interval *is* 16ms, so comparing them exactly
/// means a fraction of a millisecond of jitter skips a frame and halves the
/// rate to 30fps. Allowing a frame that's due within half a tick keeps 60
/// landing on 60.
fn repaint_due(since_last: Duration, fps: u32) -> bool {
    since_last + Duration::from_micros(500) >= frame_interval(fps)
}

/// The target gap between composited frames at `fps`.
fn frame_interval(fps: u32) -> Duration {
    Duration::from_micros(1_000_000 / fps.max(1) as u64)
}

/// How long to sleep after a tick: the shorter of the pump cadence and the time
/// left until the next frame is due.
///
/// The pump ceiling is what keeps input and PTY output responsive at low frame
/// rates — at 1fps the loop must still drain events every 16ms, not once a
/// second. The frame deadline is what makes the *rate* exact.
fn sleep_before_next_tick(since_last_render: Duration, fps: u32) -> Duration {
    let until_due = frame_interval(fps).saturating_sub(since_last_render);
    until_due.clamp(Duration::from_millis(1), RENDER_TICK)
}
...
struct Gui {
    ...
    /// Eased position of the editor caret — see [`CaretAnim`].
    caret_anim: CaretAnim,
    /// Whether the caret is still sliding toward its cell, so the loop keeps
    /// compositing frames until it arrives. Without this the animation would
    /// advance only when something *else* asked for a repaint, and a caret
    /// moved by a keystroke would freeze partway between two cells until the
    /// idle floor elapsed.
    caret_settling: bool,
...
pub fn run(app: App) -> Result<()> {
    ...
    let mut gui = Gui {
        ...
        applied_font_size,
        caret_anim: CaretAnim::default(),
        caret_settling: false,
    ...
    loop {
        ...
        gui.tick();
        // Sleep to whichever comes first: the next pump tick, or the next
        // frame's deadline. A flat `RENDER_TICK` means frames can only land on
        // 16ms boundaries, so a 100ms (10fps) target actually fires at ~112ms —
        // the readout sat at 8.9/10 purely from that quantising. Waking exactly
        // when the frame is due costs nothing extra (it's the same sleep, just
        // shorter) and lands each rate on its number.
        std::thread::sleep(gui.sleep_until_next_frame());
    }
    Ok(())
}
...
impl ApplicationHandler for Gui {
    ...
    fn window_event(
        ...
        match event {
            ...
            // A file dragged in from Finder/Explorer. What it means depends on
            // where it landed — see `App::drop_path`. One event per file, so a
            // multi-file drop arrives here several times over.
            WindowEvent::DroppedFile(path) => {
                let (col, row) = self.cursor_cell().unwrap_or((0, 0));
                self.app.drop_path(&path, col, row);
                dirty = true;
            }
...
impl Gui {
    ...
    fn tick(&mut self) {
        ...
        // tick — force an immediate repaint for either (and while one's in
        // flight) rather than waiting for the idle floor, since the user is
        // actively watching this dialog for results.
        let search_started = self.app.poll_pending_search();
        let search_finished = self.app.poll_search_results();
        if search_started || search_finished || self.app.project_search.in_flight {
            self.dirty = true;
        }
        ...
        let idle_elapsed = repaint_due(self.last_render.elapsed(), self.app.gui_fps);
        let terminal_due = self.app.terminal_repaint_due(pumped > 0);
        let decision = render_decision(
            self.dirty,
            self.caret_catchup_pending,
            terminal_due,
            idle_elapsed,
            self.caret_settling,
        );
...
fn render_decision(
    ...
    let should_render = dirty || catchup_pending || terminal_due || idle_elapsed || caret_settling;
...
mod render_decision_tests {
    ...
    /// Frames per second the loop actually reaches for a configured `fps`,
    /// simulating the real loop: sleep for [`sleep_before_next_tick`], then
    /// composite if [`repaint_due`]. `work` models the time each iteration
    /// spends pumping events and drawing, which a fixed-sleep loop turns
    /// directly into a shortfall.
    fn achieved_fps(fps: u32, work: Duration) -> f64 {
        let mut since_last = Duration::ZERO;
        let mut frames = 0u32;
        let mut elapsed = Duration::ZERO;
        while elapsed < Duration::from_secs(4) {
            let step = sleep_before_next_tick(since_last, fps) + work;
            elapsed += step;
            since_last += step;
            if repaint_due(since_last, fps) {
                frames += 1;
    ...
    /// The reported bug: every option from 5 to 60 rendered at the same ~4fps
    /// because a hardcoded 250ms floor outranked the setting, so the readout
    /// stayed at "4.0/60" whatever was picked.
    #[test]
    fn every_fps_option_is_actually_reached() {
        // A couple of ms of per-iteration work, which is what a real tick
        // costs pumping events and terminals.
        let work = Duration::from_millis(2);
        for &fps in crate::settings::fps_options() {
            let got = achieved_fps(fps, work);
            let target = fps as f64;
            assert!(
                (got - target).abs() <= target * 0.08,
                "{fps}fps configured but the loop reaches {got:.1}fps"
            );
        }
    }

    /// The reported bug, as a number. Every option from 5 up rendered at the
    /// same ~4fps because a 250ms floor outranked the setting.
    #[test]
    fn raising_the_setting_actually_raises_the_rate() {
        let work = Duration::from_millis(2);
        let rates: Vec<f64> = crate::settings::fps_options()
            .iter()
            .map(|&f| achieved_fps(f, work))
            .collect();
        for pair in rates.windows(2) {
            assert!(
                pair[1] > pair[0] * 1.2,
                "each step up the range must be a real increase, got {pair:?}"
            );
        }
        let (slowest, fastest) = (rates[0], rates[rates.len() - 1]);
        assert!(
            fastest > slowest * 40.0,
            "the range has to span 1fps to 60fps, not collapse to one rate: \
             {slowest:.1} -> {fastest:.1}"
        );
    }

    #[test]
    fn a_low_fps_still_wins_and_is_not_dragged_up() {
        // The bug in the other direction, fixed earlier and kept fixed: at
        // 1fps the window must not repaint four times a second.
        assert!((achieved_fps(1, Duration::from_millis(2)) - 1.0).abs() < 0.1);
        assert!(!repaint_due(Duration::from_millis(500), 1), "not due yet at 1fps");
        assert!(repaint_due(Duration::from_millis(1000), 1));
    }

    #[test]
    fn the_loop_still_pumps_often_even_at_one_fps() {
        // The sleep is capped at the pump cadence no matter how far away the
        // next frame is — otherwise a 1fps window would sit blind to input and
        // PTY output for a whole second at a time.
        assert_eq!(sleep_before_next_tick(Duration::ZERO, 1), RENDER_TICK);
        // And never sleeps zero, which would spin the CPU flat out.
        assert!(sleep_before_next_tick(Duration::from_secs(5), 60) >= Duration::from_millis(1));
    }

    #[test]
    fn sleep_shrinks_to_land_exactly_on_the_frame_deadline() {
        // 10fps: 100ms interval. 92ms in, the next frame is due in 8ms — sleep
        // 8ms, not a full 16ms tick that would overshoot to 108ms (the 8.9/10
        // the readout showed).
        let sleep = sleep_before_next_tick(Duration::from_millis(92), 10);
        assert_eq!(sleep, Duration::from_millis(8));
    }

    #[test]
    fn dirty_repaint_arms_exactly_one_catchup() {
        // Keystroke: dirty repaint happens, and one catch-up is scheduled.
        let d = render_decision(true, false, false, false, false);
        ...
        let d = render_decision(false, true, false, false, false);
        ...
        let d = render_decision(false, false, false, false, false);
        assert!(!d.should_render);
        assert!(!d.next_catchup_pending);
    }
    ...
    fn terminal_due_and_idle_floor_render_without_arming_catchup() {
        ...
        let d = render_decision(false, false, true, false, false);
        assert!(d.should_render);
        assert!(!d.next_catchup_pending);

        // The idle floor (toasts, labels) similarly doesn't need a catch-up.
        let d = render_decision(false, false, false, true, false);
        assert!(d.should_render);
        assert!(!d.next_catchup_pending);
    }
...
impl Gui {
        ...
        if let Some(terminal) = &mut self.terminal {
            ...
            if let (Some(window), Ok(grid)) = (&self.window, terminal.backend().size()) {
                ...
                if grid.width > 0 && grid.height > 0 {
                    ...
                    let eased = self.caret_anim.step(&self.app.gui_carets, cell_w, cell_h);
                    self.caret_settling =
                        self.caret_anim.settling(&self.app.gui_carets, cell_w, cell_h);
                    let rects: Vec<CaretRect> = eased
                        .iter()
                        .map(|&(x, y, color)| CaretRect {
                            x,
                            y,
                            w: bar_w,
                            h: cell_h,
    ...
    /// How long the loop should sleep after this tick — see
    /// [`sleep_before_next_tick`].
    fn sleep_until_next_frame(&self) -> Duration {
        sleep_before_next_tick(self.last_render.elapsed(), self.app.gui_fps)
    }
