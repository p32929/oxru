...
/// One physical screen row under word-wrap: which rope line it shows, and
/// the `[start, end)` char range within that line's own text (0 = the
/// line's first character — not counting any earlier wrapped row of it).
/// Produced by [`Buffer::wrap_rows`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct WrapRow {
    pub line: usize,
    pub start: usize,
...
pub struct Buffer {
    ...
    /// Cached `(revision, unit)` for [`Buffer::indent_unit`] — detecting it
    /// means scanning the head of the file, which is far too much to redo on
    /// every frame just to place indent guides.
    indent_unit_cache: std::cell::Cell<Option<(u64, usize)>>,
}
...
impl Buffer {
    ...
    /// How many columns one indent level is in *this* file, for placing indent
    /// guides. Editors that assume a fixed width draw guides through the middle
    /// of nothing in a 2-space file, so this infers the unit instead: the
    /// smallest positive step between the leading-whitespace widths of
    /// consecutive lines, which is exactly one level by construction.
    ///
    /// Only the head of the file is scanned — indent style is a whole-file
    /// property in practice, and the result is cached against the revision.
    /// Falls back to 4 (what Tab inserts, see [`Buffer::indent_selection`])
    /// when there's nothing to learn from, e.g. a flat or empty file.
    pub fn indent_unit(&self) -> usize {
        const SCAN_LINES: usize = 400;
        const DEFAULT: usize = 4;

        if let Some((rev, unit)) = self.indent_unit_cache.get()
            && rev == self.revision
        {
            return unit;
        }
        ...
        let mut smallest_step = usize::MAX;
        for line in self.rope.lines().take(SCAN_LINES) {
            let mut width = 0usize;
            let mut all_blank = true;
            for ch in line.chars() {
                match ch {
                    ' ' => width += 1,
                    // A tab is its own level; a tab-indented file needs no
                    // inferred unit, so treat it as the default width.
                    '\t' => width += DEFAULT,
                    '\n' | '\r' => break,
                    _ => {
                        all_blank = false;
            ...
            // Blank lines carry no indentation information: their "indent" is
            // whatever trailing whitespace happens to be there, and comparing
            // against it invents steps that aren't real.
            if all_blank {
                continue;
            }
            if let Some(p) = prev {
                let step = width.abs_diff(p);
                if step > 0 {
        ...
        let unit = if smallest_step == usize::MAX { DEFAULT } else { smallest_step };
        self.indent_unit_cache.set(Some((self.revision, unit)));
        unit
    }

    /// The width in columns of `line`'s leading whitespace, and whether the
    /// line has any non-whitespace content at all. Indent guides on a blank
    /// line continue the *surrounding* block's guides rather than stopping,
    /// which is what makes a wall of nested code read as connected.
    pub fn indent_width(&self, line: usize) -> (usize, bool) {
    ...
    /// Word-wrap `line`'s own text to `width` character cells (chars
    /// available for text, already excluding the gutter) — see
    /// [`crate::wrap::wrap_line_ranges`]. Out-of-range `line` yields a single
    /// empty range, matching [`Self::line_len_chars`]'s "past EOF is empty"
    /// convention rather than panicking.
    pub fn wrap_line_ranges(&self, line: usize, width: usize) -> Vec<(usize, usize)> {
        if line >= self.rope.len_lines() {
            return vec![(0, 0)];
        }
        let n = self.line_len_chars(line);
        let start = self.rope.line_to_char(line);
        let chars: Vec<char> = self.rope.slice(start..start + n).chars().collect();
        crate::wrap::wrap_line_ranges(&chars, width)
    }

    /// Build the physical-row layout for a viewport of up to `height` rows
    /// starting at rope line `top_line`'s own first (leftmost) visual
    /// sub-row — word-wrap scrolling always starts a line from its
    /// beginning, never mid-line. Stops early (fewer than `height` entries)
    /// at end of file.
    pub fn wrap_rows(&self, top_line: usize, width: usize, height: usize) -> Vec<WrapRow> {
        let mut out = Vec::with_capacity(height);
        let mut line = top_line;
        while out.len() < height && line < self.rope.len_lines() {
            for (s, e) in self.wrap_line_ranges(line, width) {
                if out.len() >= height {
                    break;
                }
                out.push(WrapRow { line, start: s, end: e });
            }
            line += 1;
        }
        out
    ...
    /// The physical row offset (from `top_line`'s own first row = 0) and the
    /// column within that row, for char position `col` on rope `line`, when
    /// wrapped to `width` and scrolled so `top_line` is first. `None` if
    /// `line` is before `top_line` (scrolling forward from `top_line` can
    /// never reach it — the caller already knows to scroll up instead).
    pub fn wrap_position(&self, top_line: usize, line: usize, col: usize, width: usize) -> Option<(usize, usize)> {
        if line < top_line {
            return None;
        }
        let mut row_offset = 0usize;
        for l in top_line..line {
            row_offset += self.wrap_line_ranges(l, width).len();
        }
        let ranges = self.wrap_line_ranges(line, width);
        let last = ranges.len() - 1;
        for (i, &(s, e)) in ranges.iter().enumerate() {
            if col < e || i == last {
                return Some((row_offset + i, col.saturating_sub(s)));
            }
        }
        Some((row_offset, 0)) // unreachable: ranges is never empty
    }

    /// `(total visual rows in the whole file, visual rows before line
    /// `scroll_row`)` when wrapped to `width` — everything the scrollbar
    /// needs to size and position its thumb. Computed together in one pass
    /// since both need the same per-line wrap-row counts. With wrap off
    /// (`width` effectively infinite) this reduces to `(line_count,
    /// scroll_row)`, matching the pre-wrap scrollbar exactly.
    pub fn wrap_scroll_metrics(&self, scroll_row: usize, width: usize) -> (usize, usize) {
        let mut total = 0usize;
        let mut before = 0usize;
        for l in 0..self.rope.len_lines() {
            let rows = self.wrap_line_ranges(l, width).len();
            if l < scroll_row {
                before += rows;
            }
            total += rows;
    ...
    /// The reverse of [`Self::wrap_scroll_metrics`]'s "rows before": which
    /// line contains visual row `target` (counting from the top of the
    /// file), when wrapped to `width` — how the scrollbar turns a drag
    /// position back into a `scroll_row` to jump to. Clamps to the last
    /// line for a `target` past the end.
    pub fn line_for_visual_row(&self, target: usize, width: usize) -> usize {
        let mut acc = 0usize;
...
mod tests {
    ...
    #[test]
    fn wrap_line_ranges_matches_the_lines_own_text() {
        let mut b = scratch();
        b.insert_str("hello world foo\nshort");
        assert_eq!(b.wrap_line_ranges(0, 11), vec![(0, 12), (12, 15)]);
        assert_eq!(b.wrap_line_ranges(1, 11), vec![(0, 5)], "fits on one row unchanged");
        assert_eq!(b.wrap_line_ranges(5, 11), vec![(0, 0)], "out-of-range line -> one empty range");
    }

    #[test]
    fn wrap_rows_lays_out_wrapped_lines_across_the_viewport() {
        let mut b = scratch();
        b.insert_str("hello world foo\nshort\nlast");
        // Line 0 wraps to 2 rows at width 11 ("hello world " / "foo"), so a
        // 4-row viewport starting at line 0 covers all 3 lines.
        let rows = b.wrap_rows(0, 11, 4);
        assert_eq!(
            rows,
            vec![
                crate::buffer::WrapRow { line: 0, start: 0, end: 12 },
                crate::buffer::WrapRow { line: 0, start: 12, end: 15 },
                crate::buffer::WrapRow { line: 1, start: 0, end: 5 },
                crate::buffer::WrapRow { line: 2, start: 0, end: 4 },
            ]
        );
    }

    #[test]
    fn wrap_rows_stops_early_at_end_of_file() {
        let mut b = scratch();
        b.insert_str("only one line");
        let rows = b.wrap_rows(0, 80, 10);
        assert_eq!(rows.len(), 1, "shorter than the requested height once EOF is hit");
    }

    #[test]
    fn wrap_position_resolves_the_wrapped_row_and_column() {
        let mut b = scratch();
        b.insert_str("hello world foo\nshort");
        // "foo" (line 0's second wrapped row) starts at char 12; char 13 is
        // the 'o' in "foo", one column into that row.
        assert_eq!(b.wrap_position(0, 0, 12, 11), Some((1, 0)));
        assert_eq!(b.wrap_position(0, 0, 13, 11), Some((1, 1)));
        // Line 1 comes after line 0's 2 wrapped rows, so it starts at row 2.
        assert_eq!(b.wrap_position(0, 1, 0, 11), Some((2, 0)));
        // A line before `top_line` can never be reached by scrolling forward.
        assert_eq!(b.wrap_position(1, 0, 0, 11), None);
    }
}

/// Why `path` can't be opened as an editor tab, or `None` if it can.
///
/// The single answer to "is this openable?", so the file dialog, a drop and
/// any future caller all decide the same way and phrase it the same way.
///
/// Extension alone isn't enough: [`crate::icons::is_binary`] deliberately
/// returns `false` for a name with no extension, so a downloaded executable or
/// a `.DS_Store` used to slip past it, fail deep inside `read_to_string`, and
/// report itself as an obscure UTF-8 error rather than "not supported". So the
/// content is sniffed too — a NUL byte in the first few KB is the same test
...
pub fn unsupported_reason(path: &Path) -> Option<String> {
    let name = path
        ...
        .unwrap_or_default();
    if path.is_dir() {
        return Some(format!("{name} is a folder, not a file"));
    }
    if crate::icons::is_binary(&name) {
        return Some(format!("Can't open {name} — not a text file"));
    }
    match std::fs::File::open(path) {
        Ok(mut f) => {
            ...
            if head.get(..read).is_some_and(|b| b.contains(&0)) {
                return Some(format!("Can't open {name} — not a text file"));
            }
            None
        }
        Err(e) => Some(format!("Can't open {name} — {e}")),
    }
}
...
mod support_tests {
    ...
    fn plain_text_is_supported() {
        ...
        assert_eq!(unsupported_reason(&f), None);
    }

    /// The case the old extension-only check missed entirely.
    #[test]
    fn an_extensionless_binary_is_caught_by_its_content() {
        let dir = tempfile::tempdir().unwrap();
        let f = dir.path().join("mystery");
        std::fs::write(&f, [0x7f, b'E', b'L', b'F', 0x00, 0x01, 0x02]).unwrap();
        let reason = unsupported_reason(&f).expect("a NUL byte means binary");
        assert!(reason.contains("not a text file"), "{reason}");
    }

    #[test]
    fn a_known_binary_extension_is_caught_by_its_name() {
        let dir = tempfile::tempdir().unwrap();
        let f = dir.path().join("shot.png");
        std::fs::write(&f, "not really a png").unwrap();
        assert!(unsupported_reason(&f).is_some());
    }

    #[test]
    fn a_folder_says_so_rather_than_failing_obscurely() {
        let dir = tempfile::tempdir().unwrap();
        let reason = unsupported_reason(dir.path()).expect("a folder isn't a tab");
    ...
    fn a_missing_file_explains_itself() {
        let dir = tempfile::tempdir().unwrap();
        assert!(unsupported_reason(&dir.path().join("nope.txt")).is_some());
    }

    /// An empty file has no NUL bytes and is perfectly editable.
    #[test]
    fn an_empty_file_is_supported() {
        let dir = tempfile::tempdir().unwrap();
