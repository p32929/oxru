...
impl Icons {
    pub fn new(mode: IconMode) -> Self {
        match mode {
            ...
            // Unicode/Ascii modes keep files icon-free — just the name — which
            // renders crisply in any monospace font.
    ...
    pub fn file(&self, name: &str) -> (&'static str, Color) {
        ...
        let glyph = match self.mode {
            ...
            // the cell (unlike the full-em Nerd logos, which clipped).
            IconMode::Nerd => "\u{25cf}",
            // Unicode/Ascii keep their original clean look (no per-file glyph).
            _ => self.file_default,
        };
